...
The retrieval pipeline uses hybrid search (BM25 + cosine similarity) when the
embeddings provider is configured. The mode is selected automatically:

- **Hybrid** — BM25 + cosine, fused with `HYBRID_LEXICAL_WEIGHT` (α).
- **BM25-only** — corpus stats present, no embeddings (e.g. HYBRID_ENABLED=false).
- **Lexical** — bare loader without corpus stats (tests, fallback).

Embeddings are persisted in `kb_documents` / `kb_terms` SQLite tables and
reused across restarts when the file checksum and model are unchanged. No new
dependencies are used; all math is stdlib (`math`, `array`, `collections`).

See `.claude/rules/rag.rules` for the full set of RAG implementation constraints.
...
1. Read these CLAUDE.md
2. Identify affected area
3. Load relevant rules from `.claude/rules/`
4. Apply `.claude/rules/` for execution constraints
5. Use `.claude/skills/` if a matching workflow exists

Minimum rule loading:

- Always read `.claude/rules`
- Load additional rules depending on the task:
  - `rag.rules` — retrieval pipeline, hybrid scoring, tool contract
  - `dependency-management.rules` — new dep policy
  - `migration.rules` — SQLite schema changes
  - `logging-observability.rules` — structured logging
  - `testing-execution.rules` — test execution and assertions
  - `docker.rules` — container build constraints
...
2. CLAUDE.md
3. Most specific `.claude/rules/*.rules`
