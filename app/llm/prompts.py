...
2. **Message normalisation** — :func:`build_chat_messages` assembles a
   provider-ready message list from a mix of types objects
   (:class:`~app.types.models.Message`) and plain dicts.  This lets the
...
from app.types.enums import MessageRole
...
DEFAULT_SYSTEM_PROMPT = (
    ...
    "Decline any request that is clearly unrelated to the knowledge base types — such as coding exercises, "
...
def build_chat_messages(
    ...
    Both types objects (:class:`~app.types.models.Message`) and plain dicts
...
def _normalize_message(message: Any) -> ProviderMessage:
    """Coerce a single message — types object or dict — to a provider dict.

    Mappings are shallow-copied as-is.  Attribute-based objects (e.g. types
    :class:`~app.types.models.Message`) are converted by extracting the
    ...
    TypeError
        ...
        :class:`~app.types.enums.MessageRole`, or if ``content`` is not a
...
def _normalize_role(role: Any) -> str:
    ...
    ----------
    role:
        Either a :class:`~app.types.enums.MessageRole` enum member or a
