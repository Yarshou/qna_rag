...
2. **Message normalisation** — :func:`build_chat_messages` assembles a
   provider-ready message list from a mix of domain objects
   (:class:`~app.domain.models.Message`) and plain dicts.  This lets the
...
def build_chat_messages(
    ...
    Both domain objects (:class:`~app.domain.models.Message`) and plain dicts
...
def _normalize_message(message: Any) -> ProviderMessage:
    """Coerce a single message — domain object or dict — to a provider dict.

    Mappings are shallow-copied as-is.  Attribute-based objects (e.g. domain
    :class:`~app.domain.models.Message`) are converted by extracting the
    ...
    TypeError
        ...
        :class:`~app.domain.enums.MessageRole`, or if ``content`` is not a
...
def _normalize_role(role: Any) -> str:
    ...
    ----------
    role:
        Either a :class:`~app.domain.enums.MessageRole` enum member or a
